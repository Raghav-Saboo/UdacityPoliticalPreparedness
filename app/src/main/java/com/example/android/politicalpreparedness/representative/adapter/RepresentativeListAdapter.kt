...
class RepresentativeListAdapter(private val representativeListener: RepresentativeClickListener) :
  ...
  class RepresentativeViewHolder(val binding: ListItemRepresentativeBinding) :
    ...
    fun bind(item: Representative) {
      ...
      binding.representativePicImageView.setImageResource(R.drawable.ic_profile)
    ...
    private fun showSocialLinks(channels: List<Channel>) {
      ...
      if (!facebookUrl.isNullOrBlank()) {
        enableLink(binding.facebookIconImageView, facebookUrl)
      }

      val twitterUrl = getTwitterUrl(channels)
      if (!twitterUrl.isNullOrBlank()) {
        enableLink(binding.twitterIconImageView, twitterUrl)
      }
    }

    private fun showWWWLinks(urls: List<String>) {
      enableLink(binding.worldWideWebIconImageView, urls.first())
    }
